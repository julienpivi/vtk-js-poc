...
import './xr-volume-rendering.css';
...
let {
    fileURL = '../resources/head-binary.vti',
...
HttpDataAccessHelper.fetchBinary(fileURL).then((fileContents) => {
    ...
    xrButton.addEventListener('click', () => {
        ...
        const changeSessionTypeButton = document.createElement('button');
    changeSessionTypeButton.textContent = 'Change XR type';
    changeSessionTypeButton.className = 'sessionButton'
    changeSessionTypeButton.addEventListener('click', () => {
        if(xrSessionType === XrSessionTypes.HmdVR) {
            requestedXrSessionType = xrSessionType = XrSessionTypes.MobileAR;
        }else {
            requestedXrSessionType = xrSessionType = XrSessionTypes.HmdVR;
        }
        if (navigator.xr !== undefined && xrRenderWindowHelper.getXrSupported()) {
            enterText =
                requestedXrSessionType === XrSessionTypes.MobileAR
    ...
    document.body.appendChild(changeSessionTypeButton);
});
